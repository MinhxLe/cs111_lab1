...
void 
command_new (command_t m_command, enum command_type t, int stat, 
...
void 
command_set_io (command_t c, char* file, char r)
{
  switch (r)
    {
      case '<':
...
void 
command_stream_new (command_stream_t m_command_stream)
{
...
//COPIES
void
command_stream_add (command_stream_t s, command_t* c)
{
 ...
 */
int 
precedence (char opp1, char opp2)
{
  if (opp1 == '|')
    {
      if (opp2 == '|')
 ...
 */
void 
opp_handle_stacks (stack_t command_stack, stack_t opp_stack)
{
  ...
  if (opperand == '&') //TODO: what is status
...
void 
opp_create_simple_command (string_t buff, command_t s, unsigned int wc)
{
...
void 
opp_handle_operator (stack_t command_stack, stack_t opp_stack, char opp)
{
  char c;
...
void 
opp_add_new_command_tree (stack_t command_stack, stack_t opp_stack,
 ...
 */
void
clean_raw_buffer (string_t raw_string, string_t finished_string)
{
  //////necessary variables
  ...
  bool_t handled_new_line = TRUE;//handled new line, used to ignore new lines after, 
                                // first because you don't to handle it if it's first of the string
  ...
  if (prev_oper)
    {
      ...
      if (prev == ';')
        {  
          //removes the end semi colon cause fuck you that's why
          finished_string->v_string->n_elements--;
          finished_string->length--;
          string_append (finished_string, "\n\n", 2);
        }
      else
...
void 
parse_command_tree (string_t cln_string, command_stream_t tree){
  ...
  for (unsigned int i = 0; i < cln_string->length; i++)
    {
      ...
      if (is_command_char (curr_char) || curr_char == '\0')
        ...
        {
          //you're seeing something else that's not part of the original character anymore
          string_clear (simple_buff);
