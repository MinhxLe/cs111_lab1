...
void command_dep_new(command_dep_t com_dep, command_t c){
    com_dep->command = c;
...
void command_dep_add_dependency_by_vector(command_dep_t command, vector_t depend){

    vector_append_vector(command->dependencies, depend);
}

void command_dep_add_dependor(command_dep_t command, command_dep_t depr){
    vector_append(command->dependors, &depr);
}


void command_dep_remove_dependency(command_dep_t command, command_dep_t remove){
...
void command_dep_vector_new(vector_t seed, command_stream_t commands){
...
/*
   ...
   */
        ...
        void rec_execute_command_dep(command_dep_t root){
            pid_t child;
            int status;
            if ((child = fork()) == -1){
                //TODO ERROR
